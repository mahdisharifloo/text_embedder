...
log = Logger('post_image_downloader')
...
class Request:
    ...
    #             log.warning('Forbidden maybe expired url ')
    #             return 403
    ...
    @staticmethod
    def get_image(src):
        url = f"http://{cfg.DOWNLOADER_HOST}/get_image"
        payload = {}
        headers = {
        ...
        image_response = requests.request("POST", url, headers=headers,
                                          params=params, data=payload, verify=False, timeout=10)
        if image_response.status_code == 200:
            if image_response.text != '403':
                image_bytes = image_response.content
                raw_img = io.BytesIO(image_bytes)
                return raw_img
            else:
